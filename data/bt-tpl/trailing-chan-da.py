...
class XXXNAMEXXX(TrailingStrategy):
    ...
    def init(self):
        ...
        ## Resample
        # Hours
        self.hlow = resample_apply('H', ta.MIN, self.data.Low, self.CHPERIOD, agg='min')
        self.hhigh = resample_apply('H', ta.MAX, self.data.High, self.CHPERIOD, agg='max')
        # Days
        self.dlow = resample_apply('D', self.LDAY, self.data.Close)
    ...
    def next(self):
        ...
        # Calculate channel entry price
        entPrice = self.hlow[-1] + ((self.hhigh[-1] - self.hlow[-1]) * (0.01 * self.CHPERC))

        # Test AI with values at idx
        if self.predEnt(idx) and not self.position and price < entPrice and price > self.dlow[-1]:
            self.buy()
...
# optstats = bt.optimize(SL=range(1,7,1))
# optstats = bt.optimize(CHPERC=range(1,20,1), CHPERIOD=range(1,12,1))
# print(optstats)
